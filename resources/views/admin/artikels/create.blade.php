<x-layout>
      <div class="bg-finlab-bg min-h-screen text-gray-800 pt-10">
          <div class="container mx-auto px-4 py-12 pt-20">
              <h2 class="text-2xl font-bold text-gray-800 mb-4 text-center">Tambah Artikel</h2>
              <form action="{{ route('admin.artikels.store') }}" method="POST" enctype="multipart/form-data" class="max-w-2xl mx-auto">
                  @csrf
                  <div class="mb-4">
                      <label for="title" class="block text-gray-700 font-semibold mb-2">Judul</label>
                      <input type="text" id="title" name="title" value="{{ old('title') }}" class="w-full border rounded-lg px-4 py-2 @error('title') border-red-500 @enderror" required>
                      @error('title')
                  ...
                  <div class="mb-4">
                      ...
                      <input type="text" id="author" name="author" value="{{ old('author') }}" class="w-full border rounded-lg px-4 py-2 @error('author') border-red-500 @enderror" required>
                      @error('author')
                          <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                      @enderror
                  </div>
                  <div class="mb-4">
                      ...
                      <textarea id="body" name="body" class="w-full border rounded-lg px-4 py-2 @error('body') border-red-500 @enderror" rows="8" required>{{ old('body') }}</textarea>
                      @error('body')
                          <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                      @enderror
                  </div>
